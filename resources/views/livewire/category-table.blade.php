<div>
    <section class="justify-between flex max-sm:flex-col-reverse max-sm:mb-5">
        ...
        <div class="my-5">
            <button data-hs-overlay="#modal-create-kategori" class="bg-blue-600 text-white font-medium px-4 py-2 rounded-md hover:bg-blue-700">+ Tambah Data</button>
        </div>
    </section>
            ...
            <tbody class="dark:bg-meta-4">
                @foreach ($data as $item)      
                    <tr>
                        ...
                        <td class="text-center border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                            <p class="text-black dark:text-white">{{$item->kode_kategori}}</p>
                        </td>
                        <td class="mx-auto px-4 py-5 border-b border-[#eee] dark:border-strokedark">
                            <div class="flex items-center mx-auto justify-center">
                                <button data-hs-overlay="#modal-edit-kategori" type="button" id="edit-kategori"
                                    data-id-kategori="{{$item->id}}"
                                    data-nama-kategori="{{$item->nama_kategori}}"
                                    data-kode-kategori="{{$item->kode_kategori}}"
                                    class="edit-kategori transition ease-in-out hover:bg-amber-50 focus:bg-amber-50 hover:text-amber-500 focus:text-amber-500 inline-flex w-fit rounded-l-md p-2 text-gray-900 items-center hover:ring-1 ring-inset ring-gray-300 hover:ring-amber-500 focus:ring-2 focus:ring-amber-500 sm:text-sm sm:leading-6">
    ...
    {{$data->links()}}
    @include('pages.dashboard.category.edit')
