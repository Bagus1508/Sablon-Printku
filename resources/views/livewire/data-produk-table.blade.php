<div>
    <section class="justify-between flex max-sm:flex-col-reverse max-sm:mb-5">
        ...
        <div class="my-5">
            <button data-hs-overlay="#modal-create-produk" class="bg-blue-600 text-white font-medium px-4 py-2 rounded-md hover:bg-blue-700">+ Tambah Barang</button>
        </div>
    </section>
            ...
            <tbody class="bg-slate-200 dark:bg-meta-4">
                @foreach ($data as $item)
                    <tr>
                        ...
                        <td class="text-center border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                            <p class="text-black dark:text-white">{{@$item->kode_produk}}</p>
                        </td>
                        <td class="text-center border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                        ...
                        <td class="text-center border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                            <p class="text-black dark:text-white">{{$item->stok->stok_keluar ?? 0}}</p>
                        </td>
                        <td class="text-center border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                            <p class="text-black dark:text-white">{{$item->stok->sisa_stok ?? 0}}</p>
                        </td>
                        <td class="mx-auto px-4 py-5 border-b border-[#eee] dark:border-strokedark">
                            <div class="flex items-center mx-auto justify-center">
                                <button data-hs-overlay="#modal-edit-produk" type="button" id="edit-produk"
                                    data-id-produk="{{@$item->id}}"
                                    data-kode-produk="{{@$item->kode_produk}}"
                                    data-nama-produk="{{@$item->nama_barang}}"
                                    data-id-kategori="{{@$item->kategori->id}}"
                                    class="edit-produk transition ease-in-out hover:bg-amber-50 focus:bg-amber-50 hover:text-amber-500 focus:text-amber-500 inline-flex w-fit rounded-l-md p-2 text-gray-900 items-center hover:ring-1 ring-inset ring-gray-300 hover:ring-amber-500 focus:ring-2 focus:ring-amber-500 sm:text-sm sm:leading-6">
    ...
    {{$data->links()}}
    @include('pages.dashboard.data_produk.edit')
    @include('pages.dashboard.data_produk.delete')
</div>