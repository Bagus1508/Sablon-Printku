<div wire:ignore.self id="modal-update-status-spk"
        ...
        <div class="p-4 sm:p-7">
            <div class="rounded-md border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                <div
                    ...
                    <div>
                        <button data-hs-overlay="#modal-update-status-spk" type="button"
                ...
                <form method="POST" action="{{route('monitoring-kontrak-global.updateStatusSpk', (int)$ID)}}" id="form-update-status-spk">
                    @csrf
                    <div class="p-6.5">
                        <input type="text" hidden name="id" id="id_kontrak_global">
                        <div class="mb-4.5 w-full">
                            <label class="mb-3 block text-sm font-medium text-black dark:text-white">
                                No Kontrak Takon
                            </label>
                            <input type="text" id="no_kontrak_takon" name="no_kontrak_takon" disabled
...
<script>
$('.update-status-spk').click(function() {
    const id = $(this).data('id-kontrak-global');
    const no_kontrak_takon = $(this).data('no-kontrak-takon');
    const status_spk = $(this).data('status-spk');

    $('#id_kontrak_global').val(id);
    $('#no_kontrak_takon').val(no_kontrak_takon);
    $('#status_spk').val(status_spk);

    const url = '{{ route("monitoring-kontrak-global.updateStatusSpk", ":id") }}'.replace(':id', id);
    $('#form-update-status-spk').attr('action', url);
});
