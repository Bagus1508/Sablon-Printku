<div wire:ignore.self id="modal-edit-bast"
        ...
        <div class="p-4 sm:p-7">
            <div class="rounded-md border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                <div
                    ...
                    <div>
                        <button data-hs-overlay="#modal-edit-bast" type="button"
                ...
                <form method="POST" action="{{route('bast.store')}}" id="form-proses-cutting">
                    @csrf
                    <div class="px-6.5 pb-6.5">
                        <input type="text" hidden name="id_kontrak_rinci" id="id_kontrak_rinci_bast">
                        <input type="text" hidden name="id" id="id_bast">
                        <div class="mb-4.5 w-full">
                            <label class="mb-3 block text-sm font-medium text-black dark:text-white">
                                No
                            </label>
                            <input type="text" id="no_bast" name="no" placeholder="Masukan No"
                        ...
                        <div class="mb-4.5 w-full">
                            ...
                            <input type="date" id="tanggal_bast" name="tanggal_bast" placeholder="Masukan Tanggal BA RIKMATEK"
                                class="w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary" />
                        </div>
                    </div>
...
<script>
    ...
    $('.proses-bast').click(function() {
    const id_kontrak_rinci = $(this).data('id-kontrak-rinci');
    const id_bast = $(this).data('id-bast');
    const no_bast = $(this).data('no-bast');
    const tanggal_bast = $(this).data('tanggal-bast');
    
    // Mengatur nilai input ID pada form modal
    $('#id_kontrak_rinci_bast').val(id_kontrak_rinci);
    $('#id_bast').val(id_bast);
    $('#no_bast').val(no_bast);
    $('#tanggal_bast').val(tanggal_bast);
