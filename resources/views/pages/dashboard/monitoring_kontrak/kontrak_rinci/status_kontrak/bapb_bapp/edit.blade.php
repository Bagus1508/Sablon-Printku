<div wire:ignore.self id="modal-edit-bapb-bapp"
        ...
        <div class="p-4 sm:p-7">
            <div class="rounded-md border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                <div
                    ...
                    <div>
                        <button data-hs-overlay="#modal-edit-bapb-bapp" type="button"
                ...
                <form method="POST" action="{{route('bapb-bapp.store')}}" id="form-proses-cutting">
                    @csrf
                    <div class="px-6.5 pb-6.5">
                        <input type="text" hidden name="id_kontrak_rinci" id="id_kontrak_rinci_bapb_bapp">
                        <input type="text" hidden name="id" id="id_bapb_bapp">
                        <div class="mb-4.5 w-full">
                            <label class="mb-3 block text-sm font-medium text-black dark:text-white">
                                No
                            </label>
                            <input type="text" id="no_bapb_bapp" name="no" placeholder="Masukan No"
                        ...
                        <div class="mb-4.5 w-full">
                            ...
                            <input type="date" id="tanggal_bapb_bapp" name="tanggal_bapb_bapp" placeholder="Masukan Tanggal BA RIKMATEK"
                                class="w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary" />
                        </div>
                    </div>
...
<script>
    ...
    $('.proses-bapb-bapp').click(function() {
    const id_kontrak_rinci = $(this).data('id-kontrak-rinci');
    const id_bapb_bapp = $(this).data('id-bapb-bapp');
    const no_bapb_bapp = $(this).data('no-bapb-bapp');
    const tanggal_bapb_bapp = $(this).data('tanggal-bapb-bapp');
    
    // Mengatur nilai input ID pada form modal
    $('#id_kontrak_rinci_bapb_bapp').val(id_kontrak_rinci);
    $('#id_bapb_bapp').val(id_bapb_bapp);
    $('#no_bapb_bapp').val(no_bapb_bapp);
    $('#tanggal_bapb_bapp').val(tanggal_bapb_bapp);
